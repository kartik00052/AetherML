...
--------------------------------------------------------------------------------
  ...
  TEST (110/110 passed)
    [+] test_imports (0.10s) -- {'version': '0.2.0', 'all_exports': 54}
    ...
    [+] test_pipeline_order (0.50s) -- {'stages': ['upload', 'etl', 'validation', 'eda', 'target_detection', 'feature_engineering', 'model_
    ...
    [+] test_polars_engine (0.14s) -- {'shape': (20, 5), 'columns': ['age', 'income', 'score', 'category', 'target'], 'head_rows': 3, 'col
    ...
    [+] test_excel_loading (0.20s) -- {'format': 'excel', 'sheets': [{'name': 'Sheet1', 'index': 0, 'rows': 2, 'cols': 2}], 'rows': 2}
    ...
    [+] test_target_detection_regression (0.01s) -- {'target': 'sqft', 'task': 'ambiguous', 'confidence': 0.4}
    ...
    [+] test_train_classification (1.20s) -- {'best_model': RandomForestClassifier(max_depth=5, n_estimators=50, random_state=42), 'best_score':
    [+] test_train_regression (0.57s) -- {'best_model': LinearRegression(), 'best_score': -0.11661169350545464}
    [+] test_train_with_cv (0.27s) -- {'best_model': LogisticRegression(C=0.01, max_iter=200, random_state=42), 'cv_results': True}
    [+] test_evaluate_classification (0.10s) -- {'metrics': ['accuracy', 'precision_macro', 'recall_macro', 'f1_macro', 'roc_auc', 'confusion_matrix
    [+] test_evaluate_regression (0.13s) -- {'metrics': ['rmse', 'mae', 'r2'], 'task_type': 'regression'}
    [+] test_shap_explainability (0.35s) -- {'explainer_type': 'TreeExplainer', 'features': 4, 'sampled': False}
    ...
    [+] test_report_generation (0.01s) -- {'md_length': 672, 'html_length': 2128}
    ...
    [+] test_simple_api_select_model (2.91s) -- {'result_type': 'ModelResult'}
    [+] test_simple_api_train (3.13s) -- {'result_type': 'TrainResult'}
    [+] test_simple_api_explain (2.77s) -- {'result_type': 'ExplainResult'}
    [+] test_simple_api_report (2.63s) -- {'result_type': 'str'}
    [+] test_oop_api (0.01s) -- {'load_result': 'Phronesis'}
    [+] test_oop_api_incremental (2.72s) -- {'incremental_stages': 9}
    [+] test_advanced_api_full_pipeline (3.10s) -- {'target': 'target', 'task': 'ambiguous', 'model': 'RandomForestClassifier'}
    [+] test_advanced_api_subset_stages (0.02s) -- {'stages_run': 4, 'has_profile': True}
    [+] test_advanced_api_with_config (2.73s) -- {'target': 'target', 'task': 'ambiguous'}
    ...
    [+] test_cli_info (0.88s) -- {'returncode': 0, 'stdout_len': 162}
    [+] test_cli_run (5.08s) -- {'returncode': 0, 'stdout_len': 7064, 'stderr_len': 396}
    [+] test_fastapi_app_creation (0.16s) -- {'app_title': 'Phronesis', 'version': '0.2.0'}
    [+] test_fastapi_health_endpoint (0.00s) -- {'status': 'ok'}
    [+] test_fastapi_version_endpoint (0.02s) -- {'version': APIResponse(success=True, data={'version': '0.2.0', 'python': '3.11.9 (tags/v3.11.9:de54
    ...
    [+] test_stub_agent_raises (0.02s) -- {'raised': True}
    ...
    [+] test_clustering_kmeans (1.97s) -- {'algorithm': 'kmeans', 'n_clusters': 2, 'silhouette': True}
    ...
    [+] test_logging_structure (0.00s) -- {'phronesis_logger_count': 66, 'logger_names': ['phronesisml.sdk', 'phronesisml', 'phronesisml.simpl
    [+] test_no_print_statements_in_core (0.11s) -- {'print_statements_in_core': 0}
